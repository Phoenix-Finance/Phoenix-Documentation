...
The FinNexus Protocol for Options supplies a decentralized way for writing and trading options. All the functions of options - writing, trading, exercising, etc. - are deployed on smart contracts. It is a permissionless, censorship-resistant, and non-custodial protocol. The settlement is verified on-chain.

FinNexus plans to introduce its options protocol with a two-step plan for 2020. The first phase for the FinNexus Protocol for Options (FPO) will debut online today with the V0.1 release. As you should be able to tell from the version number, this release is very much a beta product.
The second phase for FPO is currently under development. In general, FPO V0.1 is a tokenized options protocol for minting and trading options tokens, while the next iteration will be a collective pooled options protocol for options writing, exercising, and trading. Different smart contracts will be deployed in these two phases. Both models will be launched on Wanchain first, and then the product as refined during this second phase will be integrated on to other chains like Ethereum and [Elrond](https://elrond.com/).

Here in this article, we will mainly discuss the upcoming FPO V0.1 model.
